...
def generate_data(n_samples, g_x_true=10.0, alpha_x_true=13.0, beta_x_true=0.7):
    ...
    t.requires_grad_(True)  # Ensure t tracks gradients
    x = torch.sin(t)
    x.requires_grad_(True)  # Ensure x tracks gradients
    g_x = torch.full_like(t, g_x_true)

    # Calculate the first derivative of alpha_x_true * x with respect to t
    ...
    # Calculate the second derivative of alpha_x_true * x with respect to t
    d2_alpha_x_dt2 = torch.autograd.grad(outputs=d_alpha_x_dt, inputs=t,
                                        grad_outputs=torch.ones_like(d_alpha_x_dt),
                                        create_graph=True)[0]

    # Compute a_x_obs as double differentiation of alpha_x_true * x wrt t plus beta_x_true * g_x
...
def loss_fn(model, t, g_x, a_x_obs):
    def predict(t):
        return model(t)[0]  # Only need a_x prediction
    ...
    # Predicted acceleration and residual loss
